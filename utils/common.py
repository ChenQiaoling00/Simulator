...
def cal_block_p_elem(h, multiple_of, mlp_ratio):
    ...
    mlp_p_elem = (h * mlp_hidden_features) * 3 * expert
    dropout1 = 0
    dropout2 = 0
    total=norm1_p_elem + norm2_p_elem + MHA + out_proj + mlp_p_elem + dropout1 + dropout2
    # print(f'norm1_p_elem: {norm1_p_elem},norm2_p_elem: {norm2_p_elem},MHA:{MHA},out_proj: {out_proj},mlp_p_elem: {mlp_p_elem},dropout1: {dropout1},dropout2: {dropout2}')
    return total, mlp_p_elem


def cal_model_p_elem(h, l, vocab_size, multiple_of, mlp_ratio):
    embedding_p_elem = vocab_size * h
    block_p_elem,mlp_p_elem = cal_block_p_elem(h, multiple_of, mlp_ratio)
    block_p_elem=l*block_p_elem
    mlp_p_elem=mlp_p_elem*l
    norm_p_elem = h
    head_p_elem = vocab_size * h
    return (embedding_p_elem + block_p_elem + norm_p_elem + head_p_elem),mlp_p_elem
...
def get_model_config(model_size):
    ...
    model_p_elem,mlp_p_elem = cal_model_p_elem(h=h, l=l, vocab_size=vocab_size, multiple_of=multiple_of, mlp_ratio=mlp_ratio)
